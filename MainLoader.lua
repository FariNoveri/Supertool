...
local categoryFrame, contentFrame
...
local defaultFramePos = UDim2.new(0.5, -300, 0.5, -250)
...
local function createJoystick()
    ...
    corner3.Parent = cameraControlFrame
...
local function switchCategory(category)
    ...
    if category == "Movement" then
        createButton("Toggle Fly", toggleFly, function() return flying end).Parent = contentFrame
        createButton("Toggle Noclip", toggleNoclip, function() return noclip end).Parent = contentFrame
        createButton("Toggle Speed", toggleSpeed, function() return speedEnabled end).Parent = contentFrame
        createButton("Toggle Jump", toggleJump, function() return jumpEnabled end).Parent = contentFrame
        createButton("Toggle Water Walk", toggleWaterWalk, function() return waterWalk end).Parent = contentFrame
        createButton("Toggle Rocket", toggleRocket, function() return rocket end).Parent = contentFrame
        createButton("Toggle Spin", toggleSpin, function() return spin end).Parent = contentFrame
        createButton("Toggle God Mode", toggleGodMode, function() return godMode end).Parent = contentFrame
    elseif category == "Visual" then
        createButton("Toggle Freecam", toggleFreecam, function() return freecam end).Parent = contentFrame
        createButton("Return to Character", returnToCharacter, function() return false end).Parent = contentFrame
        createButton("Cancel Freecam", cancelFreecam, function() return false end).Parent = contentFrame
        createButton("Teleport Character to Camera", teleportCharacterToCamera, function() return false end).Parent = contentFrame
        createButton("Toggle Hide Nickname", toggleHideNick, function() return nickHidden end).Parent = contentFrame
        createButton("Toggle Random Nickname", toggleRandomNick, function() return randomNick end).Parent = contentFrame
        createButton("Set Custom Nickname", setCustomNick, function() return false end).Parent = contentFrame
    elseif category == "Teleport" then
        ...
        createButton("Teleport to Player", teleportToPlayer, function() return false end).Parent = contentFrame
        createButton("Teleport to Spawn", teleportToSpawn, function() return false end).Parent = contentFrame
        createButton("Save Position 1", function() savePosition(1) end, function() return false end).Parent = contentFrame
        createButton("Save Position 2", function() savePosition(2) end, function() return false end).Parent = contentFrame
        createButton("Load Position 1", function() loadPosition(1) end, function() return false end).Parent = contentFrame
        createButton("Load Position 2", function() loadPosition(2) end, function() return false end).Parent = contentFrame
        ...
        local function updatePlayerDropdown()
            ...
            playerDropdown.MouseButton1Click:Connect(function()
                ...
                playerDropdownFrame.Size = playerDropdownFrame.Visible and UDim2.new(1, 0, 0, #playerNames * 45) or UDim2.new(1, 0, 0, 0)
            end)
        end
        updatePlayerDropdown()
    elseif category == "Macro" then
        createButton("Toggle Record Macro", toggleRecordMacro, function() return macroRecording end).Parent = contentFrame
        createButton("Toggle Play Macro", togglePlayMacro, function() return macroPlaying end).Parent = contentFrame
        createButton("Toggle Auto Play Macro on Respawn", toggleAutoPlayOnRespawn, function() return autoPlayOnRespawn end).Parent = contentFrame
        createButton("Toggle Record Macro on Respawn", toggleRecordOnRespawn, function() return recordOnRespawn end).Parent = contentFrame
    end
end
...
local function createButton(text, callback, toggleState)
    ...
    button.BackgroundColor3 = toggleState() and Color3.fromRGB(0, 150, 0) or Color3.fromRGB(60, 60, 60)
...
local function createDropdown(text, items, callback)
    ...
    dropdownFrame.Position = UDim2.new(0, 5, 0, 50)
...
-- Create GUI with left panel categories
local function createGUI()
    local success, errorMsg = pcall(function()
        ...
        titleCornerFix.Size = UDim2.new(1, 0, 0, 25)
        titleCornerFix.Position = UDim2.new(0, 0, 1, -25)
        titleCornerFix.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
        titleCornerFix.BorderSizePixel = 0
        titleCornerFix.ZIndex = 11
        ...
        title.Text = "Krnl UI - Advanced Script"
        title.ZIndex = 12
        title.Parent = titleBar

        -- Category frame (left panel)
        categoryFrame = Instance.new("ScrollingFrame")
        categoryFrame.Size = UDim2.new(0, 150, 1, -60)
        categoryFrame.Position = UDim2.new(0, 5, 0, 55)
        ...
        local categories = {"Movement", "Visual", "Teleport", "Macro"}
        ...
        for _, categoryName in pairs(categories) do
            ...
            categoryButton.BackgroundColor3 = categoryName == currentCategory and Color3.fromRGB(0, 150, 255) or Color3.fromRGB(50, 50, 50)
        ...
        -- Update canvas size for category frame
        categoryLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
            categoryFrame.CanvasSize = UDim2.new(0, 0, 0, categoryLayout.AbsoluteContentSize.Y + 10)
        end)
        categoryFrame.CanvasSize = UDim2.new(0, 0, 0, categoryLayout.AbsoluteContentSize.Y + 10)
